...
### Player status: active / restricted / disabled

Three states, derived by **`playerStatus(player)`** (`gameLogic.ts`) from **two independent booleans** on the player record — `disabled` and `restricted`. It is deliberately NOT a single `status` string: `disabled` is load-bearing in a dozen callables, the ban sweep and the Auth kill-switch, and re-keying it would be a wire migration for no gain. The UI (the Players card's segmented control) sets **exactly one flag at a time**, so a player who went restricted → disabled → active does not silently come back restricted, and `disabled` outranks `restricted` for display either way.

**`restricted` is one rule and one rule only: the player does not get a claim back early.** Everyone else's mission claim / tile adventurer is released the moment *their own* slots go terminal (see the pooled-claims note under Guildmaster Missions); a restricted player's is held until the **world itself resolves** — the mission or tile completing frees it, via the same completion paths that run for everybody (`completeMission`, `awardTileRewards`). That is the pre-pooled-claims behaviour, applied to one player as a penalty. Nothing else changes: they log in, enlist, play, and settle normally.

- **The gate is `releasesClaimsEarly(player)`**, and every early-release site calls it: the two tile syncs (`ChallengesPage`, `MapPage`), the admin slot edit (`GameStateProvider.adminSetAdventurerSlots`), and the mission sync (`MissionsPage`). `tickSlotStatuses` inlines the same check for **both** its tile and mission blocks, the way it already inlines `deriveStatus` — **a change to one must be made in both.** Seat *removals* (fold, stand-down, kick, void, the deadline sweep) free the claim regardless and are deliberately untouched: the player has left the world.
- **`setPlayerRestricted` is a CLIENT write**, unlike disable. There is nothing to enforce Auth-side — no token capability is gated on it — and enforcement lives entirely in the release sites, one of which is already the server's scheduled tick. Cleared by writing **null, not `false`**, so the tri-state never leaves a dead key.
- **Rules**: admin-only *by construction*. `players/$playerId` has no `.write` at or above the `restricted` leaf except the season's admin one (the player-writable leaves are `adventurers` / `nameColor` / `feats`), so a player cannot lift their own penalty; the `.validate` is a shape guard, not the boundary. Pinned in `tests/rules/seasons.rules.test.ts`.
- **Say so where it bites.** A restricted player who finished their slots and still can't sit down would otherwise just see a broken button. `computeMissionCard` takes a `restricted` flag purely to swap the no-free-claim copy (the default *"finish your part of it to free your claim"* is a lie for them), and `CasinoShell` renders a `lockNote` above the open tables.

> **The admin Players roster sorts into three claim-activity bands, then Discord username** (`comparePlayersForAdmin` / `claimActivity` in `gameLogic.ts`), each drawn under its own heading: **active** (In Play) → **settled** (Played This Season) → **none** (Not Yet Playing). The band tracks the **TABLE, not the claim** — a settling seat, whose claim came back the moment its own slots went terminal, is still `active`, because the question the roster answers is "who is still at a table". The same on the tile side, which is why the tile scan reads `tile.state` (`available`/`inprogress` = live, `complete` = settled) rather than the adventurer's `busy` flag: an early-released adventurer has no `busy` left to read. Held vs freed is shown by the card's own `· settling` tag, not the band. `PlayersPage` cuts the bands as contiguous runs off the already-sorted list, so the headings can never disagree with the order, and their counts follow the filter.
...
> **`100%` is a THIRD state, not a synonym for done — the two last-player warnings put it on opposite sides.** A slot at `100%` has every check but no goal, so `statusReport.ts` keeps two predicates and neither may be collapsed into the other (or into `FREE_COMPLETED_STATUSES`, which counts `100%` as complete only because it releases the player's *claim*): **`seeking`** = `Unstarted | In-Progress` (still finding checks; a 100% player is **not** seeking) and **`ungoaled`** = those plus `100%` (world can't close; a 100% player **is** ungoaled). From them: **`lastPlayer`** — no other player ungoaled, i.e. everyone else goaled (fires on `In-Progress` *and* `100%`, so a lone 100% player no longer makes the whole world vanish from the report); **`lastChecker`** — no other player seeking but ≥1 ungoaled, i.e. the others sit at 100% and are most likely blocked on an item only this player can still send. The two are mutually exclusive by construction. `100%` deliberately trips **no** time-based flag (`stalled` / `noActivity144` / `allIdle60` stay `In-Progress`-only) — a player with every check has nothing to nag about.
...
> **The table link MUST carry `?seasonId=`.** The mini-app has no `SeasonProvider`, so the URL is the only way it learns its season; without the param it falls back to `config/activeSeasonId` and looks for the mission in the wrong season, reporting "Mission not found or unavailable." All three link builders pass it: `PhasePanel.tableHref`, `GuildmasterMissions.CasinoTableLink`, and the admin 🎰 link on `MissionsPage` (which is how the host opens a table to use the seat-card reveal below).
...
> **Host view — seeing what a seat DIDN'T commit** (`adminGetCasinoHands`): committed cards are already public (`lockedCards`), but the rest of a seat's pool is not, and that half is what answers "if I deny this config, what else could they pick?". The pool lives in `seasonSecrets/`, which is **owner-read only with no admin exception** (a read grant there cascades to every seat's hand), so the admin reads it through this callable — assembling the pool exactly as the table does: Hold 'Em = persisted `hole` + public `community` through `holdemPool()`, every other game = the retained `hand`. It is surfaced in the **table mini-app**, not the admin tab: the 🎰 link opens the table, and a **Host** bar above the seat rail toggles each seat's cards inline (committed marked, the rest dimmed). A **claimed** card came off another player's deck — a different uid namespace — so it is identified by its index-aligned slot's `claimed` flag, **never by uid**, which can coincide across the two. Secrets are purged at settle, so a settled table reports committed cards only (`poolKnown: false`).

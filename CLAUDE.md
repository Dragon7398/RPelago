...
| `src/lib/missionLogic.ts` | Mission card computation, decay/deploy logic, `currentMaxSlots`, `seatTally` (display seat count), `computeMissionCard`, `freshMission`, the sourced-game helpers (`sourcedGameLists`, `gameNoveltyInYaml`, `sourcedAt`, `gameTitleKey`), the admin triage helpers (`missionPendingAction`, `missionReadyToComplete`, `seatOwesConfig`, `seatsAwaitingConfig`, `seatsMissingConfig`, `outstandingConfigsBlockRoom`, `missionClockOrigin`, `compareMissionsForAdmin`), and the weighted casino pot split (`casinoPotShares`, `casinoTableShares`, `seatPotWeight`, `casinoShareDenominator`) |
